...
\rosa{ Utilizando a propriedade de multiplicação de expoentes temos que:}\\
\rosa{ $((-5)^{3.2}= (-5)^6= (-5) \cdot (-5) \cdot (-5) \cdot (-5) \cdot (-5) \cdot (-5)= 15.625 kg)$}\\
...
\reduline{ de escrever seria $(8 \cdot 10^9)$.\hfill}\\
...
Com as informações obtidas pelo matemático, resolva o que se pede a seguir.
...
\conteudo{Comparar dois números racionais significa estabelecer se entre eles
existe uma relação de superioridade, inferioridade ou igualdade.

Qualquer número racional negativo é menor que qualquer número racional positivo.
...
Calcule, então, a porcentagem dos alunos que escolheram
...
\rosa{ $J = 2450 \cdot 0,0196 \cdot 2$}
...
$ax^2 + b = 0$, em que $a$, $b$ e $c$ são constantes reais, e 
$a$ não pode ser igual a zero, pois isso resultaria em uma equação linear. A incógnita 
...
\rosa{Não ha solução no conjunto R.}
...
\rosa{$4y^2 - 9 = 0 \rightarrow y = \pm \sqrt{\frac{9}{4}}$}
...
\rosa{d) $(\left(- \frac{5}{3}\right), (\frac{5}{3}))$}
...
\rosa{Realizando a equação, temos que:}
...
\rosa{Como a idade não pode ser representada por número negativo, temos:}
...
\rosa{Temos que: $x^2 - 5x + 4 = 0; A = 1; B = - 5; C = 4$}

\rosa{$(\frac{- ( - 5) \pm \sqrt{{- 5}^{2} - 4.\ 1.4}}{2.1})$;$(\frac{5 \pm \sqrt{25 - 16}}{2})$}

\rosa{$(\frac{5 \pm \sqrt{9}}{2})$ ; $(\frac{5 \pm 3}{2})$}

\rosa{$(x_1) = (\frac{5 + 3}{2})= (\frac{8}{2}) = 4; (x_2) = (\frac{5 - 3}{2}) = (\frac{2}{2})=1$}

\rosa{Logo, os tamanhos disponíveis de madeira são 1 metro e 4 metros.}
...
\rosa{ Utilizando a razão da densidade demográfica (Dm), temos:}
...
\rosa{ $x = 3.744$ pães são produzidos em doze horas.}
...
\conteudo{Um polígono é uma figura geométrica plana formada pela conexão
de segmentos de reta, que são chamados de \tectbf{lados}. Esses lados
nunca se interceptam, exceto nos seus extremos, que são denominados \textbf{vértices}.

Os polígonos podem ser categorizados em duas principais classes: convexos e não convexos.
...
% \begin{figure}[H]
% \centering\includegraphics[width=\textwidth]{./imgSAEB_8_MAT/media/image7.png}
% \end{figure}

% \num{3} Classifique os polígonos a seguir em convexos e não convexos.

% \begin{figure}[H]
% \centering\includegraphics[width=\textwidth]{./imgSAEB_8_MAT/media/image8.png}
% \end{figure}
...
\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{./imgSAEB_8_MAT/media/image11.png}
\end{figure}
...
\begin{figure}[H]
\centering\includegraphics[width=2.20833in]{./imgSAEB_8_MAT/media/image14.png}
\end{figure}
...
\rosa{ a) $ 3x + 2x + x = 180; 6x = 180; x = 30. Logo, os respectivos ângulos são 30°,60° e 90°.$}

\rosa{ b) $x + x + 30 + 60 = 180; 2x + 90 = 180; 2x = 90; x = 45$}

\rosa{ Log$o, os ângulos são 45°, 75° e 60°.$}

\rosa{ c) $x + x + 20 + 2x = 180; 4x + 20 = 180; 4x = 160; x = 40$}

\rosa{ Log$o, os ângulos são respectivamente 40°, 60° e 80°$}

\rosa{ d) $a = 40°; e) a = 55° f) a = 108°$}
...
\reduline{$A = 4 \cdot 4 + 2 \cdot 2$\hfill}
\reduline{$A = 16 + 4$\hfill}
...
\reduline{A área é a de três quadrados de 3 cm de lado.\hfill}
\reduline{$A = 3 \cdot 3 \cdot 3\hfill}
\reduline{$A = 27 cm^2$\hfill}
\linhas{1}
\end{escolha}
...
\rosa{ Logo, a chance de Nátali tirar o número 4 no dado}

\rosa{ e conseguir a bonificação é de $\frac{1}{6}$.}
...
\reduline{ $\frac{5!}{3! \cdot 2!} = \frac{120}{6 \cdot 2}$\hfill}\\
