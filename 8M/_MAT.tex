...
Resposta: Sim, como exemplos pode ser citado $\sqrt{2} x \sqrt{2} = \sqrt{2}^2 = 2% e também \(\frac{\sqrt{2}}{\sqrt{2}}\) = 1
...
\conteudo{Expressões algébricas são aquelas que indicam operações matemáticas que
contêm números e letras ou somente letras. É possível usar letras para representar números reais 
desconhecidos, que chamamos de incógnitas.
...
numérica (que é um número real), e a parte literal, que corresponde a uma
...
ser simplificada adicionando-se ou se subtraindo os coeficientes.
...
\chapter{Equação polinomial do 2º grau}
...
\conteudo{A teoria das equações do segundo grau é uma parte essencial da álgebra e desempenha um papel 
importante na resolução de problemas matemáticos. Nessa teoria, podemos explorar as características e as 
propriedades das equações do segundo grau sem fazer uso de números específicos.

Uma equação do segundo grau é uma equação algébrica que envolve uma variável elevada ao quadrado (potência 
2), multiplicada por um coeficiente, e outras variáveis elevadas ao primeiro grau (potência 1) e um termo 
constante. Uma forma geral de uma equação do segundo grau é dada por $$ax\textsuperscript{2}+x+c=0$$, em que
a, b e c são coeficientes conhecidos e x é a variável desconhecida que estamos procurando.

As raízes de uma equação do segundo grau são os valores da variável que satisfazem a igualdade. Esses valores 
são encontrados por meio do processo de resolução da equação. Em uma teoria abstrata, podemos expressar as 
raízes como incógnitas, representando-as como letras.

O discriminante é uma medida importante para determinar a natureza das raízes de uma equação do segundo grau. 
O discriminante é calculado como $$D=b\textsuperscript{2}-4ac$$. Com base no valor do discriminante, podemos 
inferir informações sobre as raízes da equação. Se o discriminante for maior que zero, a equação possui duas 
raízes reais e distintas. Se o discriminante for igual a zero, a equação possui duas raízes reais e iguais.
Se o discriminante for menor que zero, a equação possui raízes complexas conjugadas.
...
\seubsection{Habilidades da BNCC}

\begin{itemize}
\item EF08MA12, EF08MA13.
\end{itemize}

\conteudo{A teoria da proporcionalidade é um conceito matemático fundamental que descreve a relação entre 
duas ou mais variáveis. Ela nos permite entender como as mudanças em uma variável estão relacionadas às 
mudanças em outra variável. Existem dois tipos principais de proporcionalidade: proporcionalidade direta e 
