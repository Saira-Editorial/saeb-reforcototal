...
Uma equação do primeiro grau é toda expressão que tem incógnitas (representadas por letras) e números e
o grau das incógnitas é 1. Devemos encontrar o valor das incógnitas, isolando-as, 
para isso, utilizamos das operações básicas,
adição, subtração, multiplicação e divisão. Para isolar as incógnitas, é
...
Como tinha uma soma com a incógnita, utilizamos da operação inversa que é a
multiplicação para descobrir o valor numérico da incógnita. Se fosse uma
subtração:
...
Ou seja, tudo que está com a incógnita vai ser operado de forma inversa para
a incógnita ser isolada.
...
\medskip\hfill\includegraphics[width=.7\textwidth]{./tikz/eixos.pdf}\hfill\medskip
...
Valor restante: $R\$250,00 - R\$50,00 = R\$200,00$;
...
\conteudo{\subsubsection{Expressões algébricas}

As expressões algébricas são expressões matemáticas que relacionam
incógnitas e números, ou apenas incógnita, que representam valores
...
$$- 15x \rightarrow \text{ coeficiente} = \  - 15 \text{ e parte literal} = x$$

Como, em uma expressão algébrica, a incógnita não tem um valor fixo, ela é
...
Temos o dobro de um valor somado com $7$. Consideremos
esse valor como $5$.

$$2a + 7 = 2 \times 5 + 7 = 10 + 7 = 17 \leftarrow \text{valor numérico}$$
...
Considere as expressões $2(x\  + \ 1) + 3$ e
...
\conteudo{\subsubsection{Razão e Proporção}

\textit{Razão:} é uma relação de divisão entre duas grandezas de
...
\medskip \noindent  \textit{Proporção:} é a igualdade entre duas razões. Assim, os
números reais não nulos $A, B, C, D$ formam uma proporção. Temos a
seguinte relação:

$$\frac{A}{B} = \frac{C}{D} = K$$

Lemos que \textit{$A$ está para $B$, assim como $C$ está para $D$}, sendo $K$ a
...
\medskip \noindent  \textit{Propriedade fundamental das proporções:} em uma proporção
$\frac{A}{B} = \frac{C}{D}$, vale a propriedade:
$A \times D\  = \ B \times C$.

\medskip  \noindent \textbf{Observação:} Outra propriedade muito usada no contexto de
...
Qual é o valor da incógnita a seguir, sabendo que as
