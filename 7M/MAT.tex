...
O conjunto dos números naturais é chamado de $\mathbb{N}$. Números naturais são
...
O conjunto dos números naturais é chamado de $\mathbb{R}$.
...
\medskip \noindent \textbf{Observação:} Lembre-se de que, numa operação em que
 temos um sinal de subtração (-) antes de um número negativo (-5), eles se
 cancelam e se tornam um sinal de adição (+).
...
\num{7} Pietra, Marcelo e Antônio criaram um jogo para estudar para a prova de
matemática. O objetivo do jogo era montar uma expressão matemática que
resultasse no valor negativo mais alto possível. Quem conseguisse montar a
expressão com o maior resultado negativo seria o vencedor. Agora, vamos
determinar quem foi o vencedor entre eles.
...
\enlargethispage{2\baselineskip}
\num{3} \textls[-10]{Mateus está comprando um lote para construir uma nova sede para sua
...
extensão do muro que será construído no lote de Mateus?}\looseness=-1
\begin{multicols}{2}
\begin{escolha}[itemsep=0pt]
  \item $24\,m$
  \item $36\,m$
...
Loja Z: $10\%$ desc. = $0,9 \times 46.500,00 = 41.850,00$;

$46.500,00$ a prazo

$46.500 - 41.850 = 4.650,00$

Portanto, Mário escolheu a loja B e pagou $R\$41.850,00$ no carro.\hfill}
...
Desv. de $10\%$ (2022): $7695 \times 0,9 = 6925,50$
...
\conteudo{\vspace{-2em}\subsubsection*{Equação do primeiro grau}

Uma equação do primeiro grau é toda expressão que tem incógnitas
(representadas por letras) e números, e o grau das incógnitas é 1. Devemos
encontrar o valor das incógnitas, isolando-as, e para isso, utilizamos das
operações básicas: adição, subtração, multiplicação e divisão. Para isolar as
incógnitas, é necessário realizar manipulações de forma a manter o equilíbrio
da equação. Para isso, utilizamos as operações inversas.  
Quando temos uma equação com uma ou mais incógnitas, nosso objetivo é isolar a incógnita, ou seja, deixá-la sozinha de um lado da equação. Para fazer isso, utilizamos as operações inversas das operações presentes na equação. Vamos reescrever essa explicação de forma mais detalhada. Por exemplo, se temos a equação:

$$x + 5 = 10$$

Para isolar a incógnita $x$, precisamos eliminar o termo $5$ que está somado a ela. Para isso, utilizamos a operação inversa da adição, que é a subtração. Subtraímos $5$ de ambos os lados da equação:

$$x + 5 - 5 = 10 - 5$$

Ou seja: $x = 5$.

Agora a incógnita $x$ está isolada, pois não há mais nenhum termo somado ou subtraído a ela!
...
Quando temos uma equação com uma ou mais incógnitas, nosso objetivo, então, é isolar a incógnita e encontrar o seu valor numérico. Para isso, utilizamos as operações inversas das operações presentes na equação.

No primeiro exemplo: $x + 8 = 11$.
Para isolar a incógnita $x$, temos que eliminar o termo $8$ que está somado a ela. Utilizamos a operação inversa da adição, que é a subtração. Subtraímos $8$ de ambos os lados da equação: $x + 8 - 8 = 11 - 8$.
Simplificando, temos: $x = 3$. E assim, a incógnita $x$ está isolada e seu valor numérico é 3.
...
Representamos a idade de Rafael como $x$. A equação fica como:
$2x + 10 = 20$.
...
\reduline{$7b + 4c = 10,70$,
$1c = 25b$.

Como uma das variáveis já está isolada, basta substituir na 1a equ.:
...
\reduline{Podemos calcular o tempo necessário para João juntar o valor total necessário
dividindo o valor restante pelo valor que ele economiza por semana.
Resto: $R\$250,00 - R\$50,00 = R\$200,00$;
Tempo necessário: $R\$200,00 \div R\$10,00/semana = 20$ semanas;
Portanto, João levará $20$ semanas para juntar o valor total necessário para comprar o videogame.\hfill}
...
% coeficiente de $x$ ($a=10$, pois João economiza R\$ $10,00$ por semana), $b$ é o
% termo constante ($b=50$, pois ele já tem R\$ $50,00$) e $c$ é o valor total
% necessário para comprar o videogame ($c=250$).
...
\num{10} Uma loja vende camisetas a R\$ 30,00 cada e calças a R\$ 50,00 cada.
...
\conteudo{\vspace*{-2em}\subsubsection{Expressões algébricas}


As expressões algébricas são usadas para relacionar incógnitas e números,
representando valores desconhecidos a serem calculados.

Por exemplo, a expressão ``o dobro de um valor somado com 7'' pode ser escrita
como $2a + 7$, onde $a$ é a incógnita.

Uma expressão algébrica é composta por termos algébricos, que têm um
\textit{coeficiente} e uma \textit{parte literal}. Por exemplo, em $-15x$, o coeficiente é $-15$ e
a parte literal é $x$.

Quando atribuímos um valor específico à variável em uma expressão algébrica,
obtemos o valor numérico da expressão. Por exemplo, se considerarmos o valor
5 para $a$ na expressão $2a + 7$, o valor numérico será $17$.

Em expressões algébricas que possuem termos com a mesma parte literal, esses
termos são chamados de termos semelhantes. Podemos agrupar e reduzir esses
termos a um único termo. Por exemplo, em $5ab + 9ab - 2ab - 20ab + 8a$, os
termos semelhantes com a parte literal $ab$ podem ser combinados, resultando
em $-9ab + 8a$.

No entanto, se os termos possuem partes literais diferentes, eles não podem
ser agrupados. Por exemplo, o termo $8a$ não pode ser combinado com os termos
que possuem a parte literal $ab$.

Existem expressões algébricas equivalentes, que produzem o mesmo valor
numérico quando substituímos os mesmos valores para as variáveis. Por
exemplo, as expressões $2(x + 1) + 3$ e $2x + 5$ são equivalentes. Se
substituirmos $x$ por 4, ambas as expressões resultam em 13.

Em resumo, as expressões algébricas são usadas para representar relações
matemáticas com incógnitas e números. Elas podem ser simplificadas, agrupando
termos semelhantes, e podem ser equivalentes quando produzem o mesmo valor
numérico ao substituir as mesmas variáveis.
...
\conteudo{\vspace*{-2em}\subsubsection{Razão e Proporção}

\textit{Razão:} A razão é uma relação de divisão entre duas grandezas de algum
 sistema de medidas. Ela é representada por $r = \frac{A}{B}$, onde $A$ e $B$ são
 os valores das grandezas.

\medskip \noindent \textit{Exemplo:} Se temos a razão de 3 para 5, escrevemos $\frac{3}{5} =
 0,6$. 
...
\medskip \noindent \textbf{Observação:} Outra propriedade útil para encontrar o valor da
 constante de proporcionalidade é 
$$\frac{A}{B} = \frac{C}{D} = \frac{A \pm C}
 {B \pm D}$$

\medskip \noindent \textit{Exemplo:} 

Se temos a proporção $$\frac{x}{3} = \frac{6}{15}$$ podemos
 encontrar o valor de $x$ fazendo $15x = 18$, o que resulta em $x = 1,2$.

\medskip \noindent \textit{Exemplo:} 

Se temos a proporção $$\frac{x}{4} = \frac{y}{6} = \frac
 {x + y}{10} = \frac{25}{10} = 2,5$$ podemos encontrar os valores de $x$ e $y$.
 Como sabemos que $x + y = 25$, podemos concluir que $x = 10$ e $y = 15$.

\medskip \noindent \textit{Grandezas Diretamente Proporcionais (G.D.P):} Duas grandezas são
...
\conteudo{\subsubsection{Transformações geométricas no plano cartesiano}
...
\medskip \noindent  \textit{Translação:} A translação consiste em mover um objeto em uma determinada
...
\medskip \noindent  \textit{Rotação:} A rotação envolve girar um objeto em torno de um ponto fixo
...
\medskip \noindent  \textit{Reflexão:} A reflexão é uma transformação que espelha um objeto em
...
\conteudo{\vspace*{-2em}\subsubsection{Retas paralelas e retas transversais}

As retas paralelas são aquelas que têm a mesma direção, mas nunca se
encontram, ou seja, não possuem pontos em comum.

Uma reta transversal é aquela que intersecta simultaneamente duas ou mais
retas em pontos diferentes.

Quando as retas paralelas são cortadas por uma transversal, o plano é dividido
em regiões internas e externas às retas paralelas, bem como em lados
diferentes em relação à transversal. Nessa situação, surgem ângulos que se
relacionam de várias formas, sendo chamados de ângulos correspondentes,
alternos e colaterais.

{\ \ \ \ \ \ \ \ \ \ 
\includegraphics[width=.65\textwidth]{./tikz/paralell.pdf}}

\begin{itemize}
\item 
  Ângulos correspondentes ocupam a mesma região da reta transversal e
 regiões diferentes das retas paralelas. Esses ângulos são congruentes, ou
 seja, possuem a mesma medida.
\item
  Ângulos alternos ocupam a mesma região das retas paralelas e regiões
  diferentes da transversal. Podem ser alternos internos ou alternos
  externos, e são congruentes.
\item
  Ângulos colaterais ocupam a mesma região da reta transversal e das retas
  paralelas. Podem ser colaterais internos ou colaterais externos, e são
  suplementares, ou seja, somam 180 graus.
\end{itemize}
...
\medskip\hfill\includegraphics[width=.7\textwidth]{./tikz/triangulo.pdf}\hfill\medskip
...
\item 8,5; 10,5; 20 
\rosa{$| 10,5 - 8,5 | < \ 20\  < \ 10,5 + 8,5\  \rightarrow \ \ 2 < 20 < 19 $ o triângulo não existe}
\item 3, 6, 9 
...
\begin{escolha}[itemsep=0pt]
\item $a^2 + b^2 = c^2$ (Teorema de Pitágoras) \rosa{X}
...
\begin{escolha}[itemsep=0pt]
  \item Os ângulos internos de $P$ são congruentes aos ângulos internos de $Q$. \rosa{X}
...
\item Ângulos correspondentes são congruentes. \rosa{X}
...
\conteudo{\vspace*{-2em}\subsubsection{Deslocamento}

Compreender conceitos como trajetória, direção e sentido é fundamental para se
localizar e descrever posições e deslocamentos, tanto em ambientes menores,
como uma sala de aula ou nossa própria casa, quanto em ambientes maiores,
como uma cidade.

Para interpretar uma representação espacial, é necessário ter pontos de
referência, como um prédio alto, uma praça, um cruzamento ou uma interseção
de ruas.

Ao construir uma representação espacial, é importante ter em mente os
conceitos de trajetória, direção e sentido para indicar com precisão a
posição e o deslocamento de objetos ou pessoas, e também ter uma noção de
como transcrever a visualização tridimensional para a bidimensional. Além
disso, é essencial utilizar símbolos e legendas adequados para tornar a
representação compreensível.

O deslocamento ocorre ao longo de uma trajetória, que é o caminho percorrido
desde o ponto de partida até o ponto de chegada. Essa trajetória pode ser uma
linha reta ou uma linha curva. A direção indica a orientação em que o
deslocamento ocorre, enquanto o sentido indica a orientação em relação ao
ponto de partida.}
...
População e amostra são termos utilizados em pesquisas científicas. A população refere-se ao conjunto completo, ou seja, ao público-alvo da pesquisa. A amostra, por sua vez, é uma parte desse público-alvo que é considerada relevante para a pesquisa. É essencial garantir que a amostra represente adequadamente o que a pesquisa pretende investigar.

Por exemplo, suponha que uma pesquisa seja realizada sobre a intenção de voto para o próximo presidente da República no Brasil. Seria possível entrevistar todos os eleitores do país? Considerando o curto espaço de tempo entre a candidatura e a eleição, isso seria inviável. No entanto, é importante que a amostra seja significativa, pois ela influencia um grupo de pessoas. Nesse caso, a população da pesquisa seria composta por todos os eleitores do Brasil, enquanto a amostra corresponderia ao grupo de pessoas que respondeu à pesquisa.

\subsubsection{Variáveis quantitativas e qualitativas}

As variáveis em uma pesquisa podem ser classificadas em quantitativas ou qualitativas.

As variáveis quantitativas são aquelas que podem ser expressas em números. Exemplos comuns incluem peso, altura, notas, idade, salário e pressão arterial.

Por outro lado, as variáveis qualitativas são aquelas que não são representadas numericamente. Elas podem ser divididas em duas categorias: ordinais e nominais.

Exemplos de variáveis qualitativas incluem sexo (masculino ou feminino), cor dos olhos (azul, verde, castanho, etc.) e classe social (baixa, média, alta).

Essa classificação é útil para entender o tipo de dados que estamos lidando em uma pesquisa e como eles podem ser analisados. 


\EP[2]

\begin{itemize} 
\item \textbf{Variáveis quantitativas:}
\item Número de horas de estudo por semana
\item Temperatura em graus Celsius
%\item Quantidade de produtos vendidos
%\item Nível de satisfação em uma escala de 1 a 10
\end{itemize}
...
%\item Classificação de desempenho em "ruim", "regular", "bom" e "excelente"
\end{itemize}
...
%{\centering \small \it Gráfico de colunas}

{\textbf{Gráfico de ``pizza'':} {Com o que sonham os alunos?}}\\
\medskip\hfill\includegraphics[width=.6\textwidth]{./tikz/piechart.pdf}\hfill\medskip
% %Paulo: inserir a imagem. Disponível em:
...
% TODO: tikz: Gráfico de linhas ou histograma
...
\begin{table}[htbp]
  \centering
  {Temperatura por Dia do Mês} \medskip \\
  \begin{tabular}{lr}
    \toprule
...
\conteudo{\subsection{Unidades de medida}
...
$$1l = 1\ dm^{2}; 1m^{3} = 1000l; 1cm^{3} = 1ml$$
...
\reduline{O vol. de um cilindro é calculado pela área da base pela
altura. No caso do reservatório, temos um cilindro com alt. de 4m e raio da base de 2 metros. Portanto, o volume é dado por 

$\pi \cdot (raio)^2 \cdot alt. = \pi \cdot 2^2 \cdot 4 \cong 25,13$ mtrs cúb.

$\pi \cdot (raio)^{2} \times alt. = \pi \times 2^{2} \times {4} \cong 25,13$.\hfill}
...
\conteudo{\vspace*{-2em}\subsubsection{Probabilidade condicional}

A probabilidade condicional é um conceito importante na teoria das
probabilidades. Podemos dividir os eventos em dois tipos: {eventos
independentes} e {eventos dependentes}.

\textit{Eventos independentes} são aqueles em que a ocorrência de um evento não depende
dos eventos anteriores ou subsequentes. Um exemplo disso é a probabilidade de
tirar um ás de paus em um baralho. Nesse caso, todas as cartas estão no
baralho e há apenas uma chance em 52 de tirar um ás de paus. Portanto, a
probabilidade independente de ocorrer esse evento é calculada como:

$$P(A) = \frac{\text{número de chances de ocorrer o evento}}{\text
{espaço amostral (todas as chances possíveis)}}$$
...
Por outro lado, eventos dependentes são aqueles em que a ocorrência de um
evento afeta a probabilidade de ocorrência dos eventos subsequentes. Por
exemplo, ao jogar um dado, a probabilidade de obter um número específico
depende do resultado obtido anteriormente. Da mesma forma, ao lançar uma
moeda, a probabilidade de obter cara ou coroa também depende dos resultados
anteriores.

A regra da multiplicação é utilizada para calcular a probabilidade de
ocorrência de dois eventos independentes. Essa regra afirma que a
probabilidade conjunta de dois eventos independentes é o produto das
probabilidades individuais. Portanto:

 $$P(A,B) = P(A) \cdot P(B/A) = P (A) \cdot P(B)$$

Por exemplo, se a probabilidade de Juliana acertar duas questões consecutivas
em uma prova de matemática é $0,82$, podemos calcular a probabilidade como:

$$P(2 \text{ questões corretas}) = 0,82 \cdot 0,82 = 0,6724$$

É importante observar que a probabilidade total é sempre igual a 1 (ou 100\%).
Portanto, no exemplo anterior, podemos calcular a probabilidade de falha, ou
seja, a chance de errar cada questão, como $1 - 0,82$.

Por outro lado, eventos dependentes ocorrem quando o evento anterior
...
influencia na próxima questão. Utilizando a regra da
multip. temos: P(3 acertos em questões) = $0,9 \times 0,9 \times 0,9 = \ 0,729$.\hfill}
...
diminui as chances de cair na mesma face. Pela regra da
multipl.:
